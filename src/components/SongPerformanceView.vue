    ...
    <div v-if="showShareDialog" class="share-dialog-overlay" @click="closeShareDialog">
      <div class="share-dialog" @click.stop>
        ...
        <div class="share-dialog-content">
          ...
          <div v-if="isAdmin" class="share-dialog-admin">
            <label class="share-dialog-label">Share with user (admin):</label>
            <div class="share-to-user-row">
              <select 
                v-model="shareToUserId" 
                class="share-user-select"
                :disabled="loadingUsers"
              >
                <option value="">Select a user...</option>
                <option 
                  v-for="u in usersList.filter(u => u.uid !== user?.uid)" 
                  :key="u.uid" 
                  :value="u.uid"
                >
              ...
              <button 
                class="btn-copy-to-user" 
                @click="copySongToUserFromDialog"
                :disabled="!shareToUserId || shareToUserLoading"
              >
                {{ shareToUserLoading ? '…' : 'Copy song to user' }}
              </button>
            </div>
            <p v-if="shareToUserMessage" class="share-to-user-message" :class="shareToUserMessageSuccess ? 'success' : 'error'">
              {{ shareToUserMessage }}
            </p>
          </div>
        </div>
    ...
    <div v-if="showSongList" class="song-list-view">
      ...
      <div class="songs-grid">
        <div v-for="songItem in songs" :key="songItem.id" class="song-card" @click="openSong(songItem.id)">
          ...
          <div v-if="songItem.createdAt" class="song-card-date">
            Created: {{ formatDate(songItem.createdAt) }}
          </div>
        </div>
      </div>
...
const copyShareLinkFromDialog = async () => {
...
const copySongToUserFromDialog = async () => {
  if (!song.value?.id || !shareToUserId.value) return
  shareToUserLoading.value = true
  shareToUserMessage.value = ''
  try {
    const result = await copySongToUser(song.value.id, shareToUserId.value)
    if (result.success) {
      shareToUserMessage.value = 'Song copied to user successfully.'
      shareToUserMessageSuccess.value = true
    } else {
      shareToUserMessage.value = result.error || 'Failed to copy song.'
      shareToUserMessageSuccess.value = false
    }
  } catch (e) {
    shareToUserMessage.value = e.message || 'Failed to copy song.'
    shareToUserMessageSuccess.value = false
  } finally {
    shareToUserLoading.value = false
  }
}
...
.btn-copy-to-user:hover:not(:disabled) {
  background: #218838;
  border-color: #218838;
}

.btn-copy-to-user:disabled {
  opacity: 0.7;
  cursor: not-allowed;
}

.share-to-user-message {
  margin: 10px 0 0;
  font-size: 0.875rem;
}

.share-to-user-message.success {
  color: #28a745;
}

.share-to-user-message.error {
  color: #dc3545;
}
